...
class NFTView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AuthBlocBloc, AuthBlocState>(
      ...
      if (state is AuthStateAuthenticated) {
        final user =
            (context.watch<AuthBlocBloc>().state as AuthStateAuthenticated)
                .user;
        ...
        return Scaffold(
          floatingActionButton: FloatingActionButton(
            onPressed: (() async {
              ...
              final Uint8List bytes2 = await file.readAsBytes();
              bool successfulUpload =
                  await FirebaseConnection().uploadToStorage(bytes2, user);
              successfulUpload
                  ? Navigator.pushReplacementNamed(context, GameScreen.id)
                  : print("uplaod to firebase storage failed");
            }),
            ...
            leading: Padding(
              ...
              child: user.photoURL != null
                  ...
                  : CircleAvatar(
                      ...
                      child: const Text(/*user.displayName![0]*/ "C"),
                    ),
            ),
            actions: [
              IconButton(
